...
**경상북도 119 상황요원이 이송병원 후보를 근거와 함께 비교하는 서비스**

응급실로는 환자에게 필요한 치료와 병원의 현재 상태, 실제 이동시간을
한 화면에 모아 상황요원이 검토할 병원 후보를 빠르게 좁히도록 돕습니다.
...
응급실로는 현장 보고를 바탕으로 먼저 검토할 응급실을 찾도록 돕습니다.
...
- 확인 전화에 바로 쓸 이송 후보 요약 생성

## 사용하기 전에

- **가로 1280px 이상**의 데스크톱 브라우저를 사용하세요.
- 시연용 정보나 개인을 식별할 수 없도록 처리한 환자 정보만 입력하세요.
...
2. 올바른 검색 결과를 선택합니다.
3. 화면에 표시된 주소가 신고 위치와 일치하는지 확인합니다.
4. **`확인한 위치로 계속`**을 누릅니다.
...
- 현재 상태와 그 이유
- 카카오 경로 기준 예상 이동시간(ETA)과 거리
- 응급실 가용병상과 NEMC 기준시각
- 선택한 치료영역별 병원 보고 상태
- 병원 종별, 주소, 응급실 전화번호
- 과거 보고 이력(비식별 집계)

**`우선 검토`**는 현재 `처치 불가`가 아닌 후보만 보여 줍니다.
**`전체`**를 누르면 경로가 계산된 모든 후보가 나타납니다.
...
1. 현재 판정
2. 실제 도로 ETA
3. `hpid`(동률일 때 순서를 고정하는 기준)

후보를 훑어보기 쉽게 정한 순서일 뿐, AI가 만든 의료 추천 순위가 아닙니다.
...
과거 값은 **수용확률이나 미래 예측값이 아닙니다.** 현재 판정을 바꾸거나
병원 후보 순서에 반영하지 않습니다.
...
병원 상태 판정과 후보 정렬에는 Gemini를 쓰지 않습니다. 정해진 규칙과
공공데이터, 실제 도로 ETA로 계산합니다.
...
검색 범위를 행정구역 안에 가두지 않고 경북 환자의 실제 광역 이송권을
반영했습니다.
...
| [Google Gemini API](https://ai.google.dev/gemini-api/docs) | 환자 설명에서 치료영역과 원문 근거 추출 |
