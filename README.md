...
구해조는 위급한 순간, 환자에게 필요한 치료가 가능한 병원을 함께 찾는다는 의미를 담았습니다.
큰 `구`와 `조`가 먼저 `구조`로 읽히고, 둘 사이의 작은 붉은 표식 안 `해`가 더해져 `구해조`가 되는 중의적 워드마크입니다.
...
> 구해조는 해커톤 MVP입니다. 의료행위, 병원 자동배정, 수용 확약을 제공하지 않으며 최종 판단은 119 상황요원이 수행합니다.
...
구해조는 이 과정에서 반복되는 정보 탐색을 하나의 콘솔로 모읍니다.
